...
def En_processing(text, customized_stopwords):
...
import konlpy.tag
def Kr_processing(text, customized_stopwords=None):
    filtered_content1 = re.sub(r'[^\.\?\!\s\d\w]','',text)
    filtered_content2 = re.sub(r'[^\s\d\w]','',filtered_content1)
    twitter = konlpy.tag.Twitter()
    twitter_morphs = twitter.pos(filtered_content2)
    Noun_words = []
    ...
    return filtered_content1, Noun_words